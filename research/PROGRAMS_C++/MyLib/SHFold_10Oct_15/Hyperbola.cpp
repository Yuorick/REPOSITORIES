...
THyperbola :: THyperbola( const double a, const double b)
 ...
 THyperbola &THyperbola::operator=(const THyperbola & R)
 {
	ma  = R.ma ;
	mb  = R.mb;
	return *this ;
...
TURPointXY  THyperbola::ProjectPointOnHyperbola (TURPointXY  pntInp)
{
  ...
  const double c  = a -1.;
  TComp cmpx1,cmpx2;
  // –ешение квадраьного улавнени€ a*x*x + b*x + c =0
// возвращает
// 0 - 2 действительных некраьных корн€
// 1 - 2 действительных кратных корн€
// 2- имеетс€ по крайней мере один нулевой корень a!= 0, c=0
// 3 - 2 комплексно сопр€женных корн€
...
// нахождение точек касани€ пр€мых проведенных из заданнойт тотчки  pntInp
// к гиперболе
// pntInp -  заданна€ точка на плоскости
// OUTPUT:
// pntOut0,pntOut1 -   очки касани€
// ¬озращает к-во точек касани€
//
int THyperbola::findTangencyPoints(TURPointXY  pntInp, TURPointXY  &pntOut0, TURPointXY  &pntOut1)
{
  ...
  // нахождение точек касани€ единичной гиперболы x*x - y*y =1
...
//------------------------------------------------
// проеврка корн€ касательной гиперболы
