...
// INPUT:
// valAng - угол поворота
// pntSdvig - вектор сдвига
// valRastigenie - коэффициент растяжения
// OUTPUT:
// возвращает преобразованный полигон
TURPolygon   TURPolygon::LinTransform_(const double  valAng , const TURPointXY pntOffSet,const double valRastigenie )
{
  double arrMtxPer[4] = {0.};
  arrMtxPer[0] = cos(valAng);
  arrMtxPer[1] = -sin(valAng);
  ...
  arrMtxPer[ 3] = arrMtxPer[0];
   ...
   TURPolygon PlgTemp1 = SdvigTransform(pntOffSet);

  TURPolygon PlgTemp2 = PlgTemp1.fncLinTransform( arrMtxPer );

  TURPolygon PlgRez   =  PlgTemp2.RastiagenieTransform( valRastigenie  ) ;
