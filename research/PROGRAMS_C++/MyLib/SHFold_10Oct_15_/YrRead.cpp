...
int TYrRead::YrReadСharRow(wchar_t *FileName,const int numberRow,char *pw)
...
{  int len = wcslen(NameOfFltFile) ;	if ( !( (NameOfFltFile[len - 1] == 't') && (NameOfFltFile[len - 2] == 'l') // проверка, что
...
int TYrRead ::ReadHdrOfSetFltRastrs(const wchar_t*FolderName,const wchar_t *TypesFileNames[]
 ...
 // Чтение массивов из IKD файла
 //В IKD файле с именем  FileName поочередно записано поределенное
 // количество массивов double, которое можно извлечь из HDR файла
 // настоящая процедура извлекает iQuant элементов из каждого массива
 // начиная с номера  iQuant и складывает их в массив   pOut в том
 // в том же самом порядке, в котором они хранятся в IKD файле
 // возвращает к-во элементов которое было извлечено из каждого массива
 // это к-во не обязательно = iQuant, а может быть  меньше,
 // в случае (iStart + iQuant) > (lenArray -1)
{
...
// NameOfIdfFile - имя файла header которого читается. Расширение  должно быть.ikd
// nrows   ncols  - к-во строк и столбцов соответственно в каждом массиве
//  xllcorner  - координата x левого нижнего угла
//   yllcorner  - координата y левого нижнего угла
// cellsize    - шаг решетки
//  NODATA_value  - значение которым заполняется массив в случае отсутствия величины
// quanMass - к-во массивов хранящихся в файле
{  int len = wcslen(NameOfIkdFile) ;	if ( !( (NameOfIkdFile[len - 1] == 'd') && (NameOfIkdFile[len - 2] == 'k') // проверка, что
	 && (NameOfIkdFile[len - 3] == 'i') ) )  // указанный файл имеет расширение .ikf
	{
...
{
  int len = wcslen(NameOfLogFile) ;	if ( !( (NameOfLogFile[len - 1] == 'g') && (NameOfLogFile[len - 2] == 'o') // проверка, что
