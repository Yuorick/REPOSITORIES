...
// 6. marrStrSK_VS [5]-  z
// 7. marrStrSK_VS [6]-  угол Пси (путевой)
...
// конструктор копирования
 TBombTraj_3D ::TBombTraj_3D (const TBombTraj_3D &R)
 {
	...
	mBomb = R.mBomb ;  // АБ
	memcpy(marrStrSK_VS, R.marrStrSK_VS, 7 * sizeof(long double));


 }

 // оператор присваивания
 TBombTraj_3D TBombTraj_3D::operator=(TBombTraj_3D  R)
 {
   //	mTStart = R.mTStart;  // нач момент
	...
	// парам конструктор
	TBombTraj_3D::TBombTraj_3D (	long double Tet0  // нач угол бросания
...
///
 ...
 void TBombTraj_3D::fncFillNachalnieUsloviaVS()
{
  marrStrSK_VS[0] = mX0;
...
void TBombTraj_3D::fncCalc_F(const long double valUGor,const long double valUVert, long double *arrF)
{
  ...
  long double val_q = 0. , arrGrad_q [7] = {0.} ;;
  ...
  // long double valW =  valCy * val_q * mBomb.mMidSq   /mBomb.mMass  *  valU ;
   ///////////////////////
   arrF[0] =  marrStrSK_VS [3] * cosl(marrStrSK_VS [4])* cosl(marrStrSK_VS [6]) ;
   ...
   - mBomb.fncCy( valMach) * val_q * mBomb.mMidSq   /mBomb.mMass /  marrStrSK_VS [3] *valUVert  ;
   arrF[5] = -marrStrSK_VS [3] * cosl(marrStrSK_VS [4]) * sinl(marrStrSK_VS [6]) ;
   arrF[6] = - mBomb.fncCy( valMach) * val_q * mBomb.mMidSq   /mBomb.mMass /  marrStrSK_VS [3]/ cosl(marrStrSK_VS [4])*valUGor;
...
void TBombTraj_3D::fncCalc_F_and_dF_po_dx_and_dF_po_dU(const long double valU, long double *arrF,long double *arr_dF_po_dx,long double *arr_dF_po_dU)
{
  // вычисление нормальной виртуальной температуры  и ее градиента
  long double valTay = 0.,valDerivTay = 0. ;
  fncCalcNormTemperature(marrStrSK_VS[1],valTay, valDerivTay)  ;
...
void TBombTraj_3D::fncCalc_dF_po_dU(long double val_Cy, long double val_q, long double *arr_dF_po_dU)
{
  memset( arr_dF_po_dU, 0, 5 * sizeof( long double)) ;
...
void TBombTraj_3D::fncGradF0(long double *arr_dF0_po_dx)
{
...
void TBombTraj_3D::fncGradF1(long double *arr_dF1_po_dx)
{
...
void TBombTraj_3D::fncGradF2(long double valF2,long double valTay, long double valDerivTay , long double *arr_dF2_po_dx)
{
...
void TBombTraj_3D::fncGradF3(long double val_q , long double *arrGrad_q
...
void TBombTraj_3D::fncGradF4(long double val_q , long double *arrGrad_q
...
void TBombTraj_3D::fncCalc_q(long double valMach,long double  &val_q)
{
...
void TBombTraj_3D::fncCalcMach(long double valTay, long double valDerivTay ,long double &valMach)
{
...
void TBombTraj_3D::fncCalcMach_and_GradMach(long double valTay, long double valDerivTay
...
void TBombTraj_3D::fncCalc_q_and_Grad_q(long double  &val_q, long double *arrGrad_q)
{
  memset( arrGrad_q, 0, 7 * sizeof(long double));
...
void TBombTraj_3D::fncCalc_Cx_and_Grad_Cx(long double valMach,long double  *arrGradMach
...
void TBombTraj_3D::fncCalc_Cy_and_Grad_Cy(long double valMach,long double  *arrGradMach
...
// шаг етода эйлера на врея valStepInt
 void TBombTraj_3D::fncEilerStep(const long double valUGor,const long double valUVert, const long double valStepInt)
 {
   long double arrF[7] ={0.} ;
   // шаг интегрирования фазового вектора
   fncCalc_F( valUGor, valUVert, arrF) ;
   long double arrT0[49] = {0.},arrT1[49] = {0.};
   MatrxMultScalar(arrF, 7, 1, valStepInt,arrT0); // f * dt
   MtrxSumMatrx(arrT0, marrStrSK_VS,7, 1, arrT1) ;
   memcpy(marrStrSK_VS, arrT1, 7 * sizeof(long double)) ;   ///
