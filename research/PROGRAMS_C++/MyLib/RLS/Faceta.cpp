...
__fastcall  TFaceta::TFaceta (const TFaceta &R2)
 ...
 // парам констр  2
 __fastcall TFaceta::TFaceta(const int n)
 {
	m_n = n ;
	// расстояние между излучателями в модуле
	m_d = 2. ;
...
// оператор присваивания
  TFaceta &TFaceta::operator=(const TFaceta  &R2)
 {
	 m_n = R2.m_n ;
	 m_d= R2.m_d;
	 mLambda = R2.mLambda ;
...
void TFaceta::createDiagrGraphs(wchar_t *wchFoldName1 )
{
	...
	for (int i=0 ; i < nBuffRows; i++)
	{
	  ...
	  //	parrBuff[ i * nBuffCols + 2]= fncDiagrSimple(valWidthDgr, tet * 1000.)   ;
		parrBuff[ i * nBuffCols + 2]= fncDiagrSimple(valWidthDgr, tet )   ;
...
// СКЗ суммарного шума в диаграмме фасеты
double TFaceta::calcNoiseSKZ()
{

return mSigEmitNoise * sqrt(m_n);
}

// СКЗ суммарного шума КУ ( 1 + delta) фасеты
double TFaceta::calcAmplFactSKZ()
{

return mSigEmitAmplFact * sqrt(m_n);
}
