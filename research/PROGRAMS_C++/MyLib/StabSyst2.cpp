...
// конструктор копирования
 ...
 TStabSyst2 &TStabSyst2::operator=(const TStabSyst2  &R)
 {
   memcpy(marrA,R.marrA,4 * sizeof(double));
   memcpy(marrB,R.marrB,2 * sizeof(double));
   memcpy(marrC,R.marrC,2 * sizeof(double));
...
// вывод параметров фильтра калмана в установившийся режим динамическим способом
// матирица B = {h*h/2 h}, вариант ДИСКРЕТНОГО БЕЛОГО ШУМА В КАНАЛЕ ОБЪЕКТА с учетом  флуктуационной добавки msigm_fl в правой части

void TStabSyst2::fncStabSolDynamicFltKlm_flAdd(double  *arrKStab,double *arrPStab)
{
  ...
  int i ;
	...
	{
	  OneStepFltKlm_flAdd(arrKStab,arrPStab);
 ...
 // memcpy( arrK1,arrKStab, 4 * sizeof(double)) ;
  ...
  int iI ;
   ...
   else
   {
	 ...
	 *plenparrOut = iI ;
	 *pparrOut = (double*)realloc(*pparrOut, 6 * (*plenparrOut) * sizeof(double)) ;

	  double arrKtemp[4] = {100000,50000,50000,100000};



   for (int i = 0; i < (*plenparrOut); i++)
  {

	 OneStepFltKlm_flAdd(arrKtemp,arrPStab);//(  arrKtemp,arrKtemp1, arrP) ;
	 (*pparrOut)[ 6 * i +1 ] = arrPStab[0] ;
	 (*pparrOut)[6 * i + 2 ] = arrPStab[1] ;
	 (*pparrOut)[6 * i + 3 ] = arrKtemp[0] ;
	 (*pparrOut)[6 * i + 4 ] = arrKtemp[1] ;
	 (*pparrOut)[6 * i + 5 ] = arrKtemp[3] ;
	 (*pparrOut)[6 * i  ] = ((double)i )* mh ;

  }
}
}
// шаг пересчета фильтра Калмана c добавкой в уравнении движения в пр части по положнению (смлучай дискпр белого шума в ускорении)
void 	TStabSyst2 ::OneStepFltKlm_flAdd(double *arrK,double *arrP)
{
	// Экстраполяция
  double arrKextr[4] = {0} , arrKt1[4] = {0},arrF[4] = {0}, arrKtemp1[4] = {0.} ;
  arrF[0] = msigm_w * msigm_w * mh * mh * mh * mh/4 + msigm_fl*msigm_fl;
  arrF [ 1] =  msigm_w * msigm_w * mh * mh * mh /2 ;
  arrF [ 2] =arrF [ 1] ;
  arrF [ 3] = msigm_w * msigm_w * mh * mh ;

	MtrxMultMatrx(marrA ,2, 2, arrK,2, arrKt1) ;
	MtrxMultMatrxTransp(arrKt1,2, 2,marrA ,2, arrKtemp1) ;
	MtrxSumMatrx(arrKtemp1, arrF,2,2, arrKextr) ;
	///

	// Расчет коэфф усиления, учет замера
	arrP [0] = arrKextr[ 0 ]/( msigm_bmo * msigm_bmo + arrKextr[ 0 ]) ;
	arrP [1] = arrKextr[ 1 ]/( msigm_bmo * msigm_bmo + arrKextr[ 0 ]) ;
	arrK [0] =  arrKextr[ 0 ] - arrKextr[ 0 ] * arrKextr[ 0 ] / ( msigm_bmo * msigm_bmo + arrKextr[ 0 ]) ;
	arrK [1] =  arrKextr[ 1 ] - arrKextr[ 0 ] * arrKextr[ 1 ] / ( msigm_bmo * msigm_bmo + arrKextr[ 0 ]) ;
	arrK [2] = arrK [1] ;
	arrK [3] =  arrKextr[ 3 ] - arrKextr[ 1 ] * arrKextr[ 1 ] / ( msigm_bmo * msigm_bmo + arrKextr[ 0 ]) ;

}
// вывод параметров фильтра калмана в установившийся режим динамическим способом
// матирица B = {h*h/2 h}, вариант  с учетом  флуктуационной добавки msigm_fl в правой части
// если pparrOut == NULL, то расчитываются тоько arrKStab и arrPStab
// если pparrOut != NULL, то расчитываются pparrOut - массив коэффициентов
// усиления и корреляционных матриц,  в котором информация хранится в следующем порядке:
// P1,P2,K11,K12,K22
...
// arrPStab - коефииц усиления в установившемся режиме
 ...
 void  TStabSyst2 ::fncStabSolDynamicGolubev_flAdd(double  *arrKStab,double *arrPStab
		,double **pparrOut, int *plenparrOut)
{
  double  arrK1[4] = {1000000,250000,250000,250000};
 ...
 // memcpy( arrK1,arrKStab, 4 * sizeof(double)) ;
  ...
  int iI ;
   ...
   for (int i = 0; i < (*plenparrOut); i++)
   {
	 OneStepGolubev_flAdd(arrKtemp,arrPStab);//(  arrKtemp,arrKtemp1, arrP) ;
	 (*pparrOut)[6 * i +1 ] = arrPStab[0] ;
	 (*pparrOut)[6 * i + 2 ] = arrPStab[1] ;
	 (*pparrOut)[6 * i + 3 ] = arrKtemp[0] ;
	 (*pparrOut)[6 * i + 4 ] = arrKtemp[1] ;
		...
		,double *arrKInp,double *arrKOut,double *arrS,double *arrQ)
{
  ...
  double arrE[4] = {1,0,0,1} ;
   ...
   double arrT1[4]={0},arrT2[4] ={0},arrT3[4]={0} ;
...
// без учета флуктуационной добавки msigm_fl в правой части
void TStabSyst2 ::FltKlm_Real(   double *arrK,double * arrP)
{
   ...
   for (int i = 0; i < 10000000; i++)
  {
	...
	if ( (fabs(arrK[0] - arrKtemp [0]) < 0.00001 ) && (fabs(arrK[3] - arrKtemp [3]) < 0.00001 ))
	{
	  break;
	}
...
// Экстраполяция  корреляц матрицы фильтра Голубева на время   valExtrT
void 	TStabSyst2 ::OneStepGolubevExtrapolation(double *arrK, double valExtrT, double *arrKExtr)
{

	double sq = sqrt(arrK [3] - arrK [1] * arrK [1] /arrK [0]) ;
	arrKExtr[0] =  arrK [0] + 2 * valExtrT * arrK[1] + valExtrT * valExtrT * arrK[3] +  msigm_w * msigm_w * valExtrT* valExtrT * valExtrT * valExtrT/4 + msigm_w * valExtrT * valExtrT * valExtrT * sq;
	arrKExtr[1] =  arrK [1] + valExtrT *arrK [3] + 3 * msigm_w * valExtrT * valExtrT * sq/2 + msigm_w * msigm_w * valExtrT * valExtrT * valExtrT /2;
	arrKExtr[2] = arrKExtr[1] ;
	arrKExtr[3] =  arrK [3] + 2 * msigm_w * valExtrT * sq + msigm_w * msigm_w * valExtrT * valExtrT ;
}
...
bool TStabSyst2::CalcFuncMinimum(double fi1,double fi2,double *arrK, double *arrP,double *arrS,double *arrQ)
{
  ...
  // return false ;
  return FindRoots( arrP, arrS,arrQ, rt1,rt2);
		 ...
		 , const double LambCur, const double MuCur,  const double FiCur ,double *arrKExtend ,double * arrS)
{
  ...
  if (fabs(MuCur)< EPS)
  {
   ...
   double arrT1[4]={0},arrT2[4] ={0},arrT3[4]={0} ;
   double arrT4[4]={0},arrT5[4] ={0}
	   ,arrT8[4]={0}, arrT9[4]={0},arrT10[4] ={0},arrT11[4] = {0}
		 ...
		 , const double LambCur, const double MuCur,  const double FiCur ,double *arrKExtend ,double *arrKExtendOut)
{
  double arrL[9] = {0},arrD[3] = {0},arrP3[3] = {0}
	  ,arrT0[4] = {0},arrT1[4] = {0},arrT2[4] = {0},arrT3[4] = {0},arrT4[4] = {0},arrT5[4] = {0},arrT6[4] = {0};
  double arrE[4] = {1,0,0,1} ;
  double arrValS[2];
  arrValS[0] = valS1;
  arrValS[1] = valS2;
	  ...
	  ,double *arrK ,double *arrKRez )
{
   ...
   memcpy(arrKRez,arrK,4 * sizeof(double)) ;
	...
	double arrT0[4] = {0},arrT1[4] = {0},arrT4[4] = {0},arrT5[4] = {0},arrT6[4] = {0};
  double arrE[4] = {1,0,0,1} ;
 ...
 // MatrxMultScalar(arrT5, 2,1, MuCur,arrT6);  // arrT6 = MuCur * (E -P * CT) * B
  ...
  double arr0[9] = {0} ,arr1[9] = {0},arr2[9] = {0},arr4[9] = {0},arr6[9] = {0} ;
...
void TStabSyst2::ExtrapolStepFltrKlm_Real(const double hExtr,double *arrKInp,double *arrKextr)
{
  double arrKt1[4] , arrKtemp[4] ,arrKtemp1[4], arrF[4] = {0.};
  memcpy(arrKtemp ,arrKInp, 4 * sizeof(double)) ;
  arrF[0] = msigm_w* msigm_w* mh * mh * mh /3 ;
  arrF [ 1] =  msigm_w* msigm_w* mh * mh  /2 ;
  arrF [ 2] =arrF [ 1] ;
...
// шаг экстраполяции фильтра  Голубева c добавкой в уравнении движения в пр части по положнению
// INPUT:
//  arrK - коорел матрица ошибок оценивания на момент t
//  hExtr - шаг экстраполяции
// OUTPUT:
// arrKExtr - корреляционная матрица ошибок экстраполяции на момент t +  hExtr
 void 	TStabSyst2 ::ExtrapolStepGolubev_flAdd(const double hExtr,double *arrK,double *arrKExtr)
{
	double valTemp = arrK [3] - arrK [1] * arrK [1] /arrK [0] ;
	valTemp = fabs(valTemp) ;
	double sq = sqrt(valTemp) ;
	arrKExtr[0] =  arrK [0] + 2 * mh * arrK[1] + mh * mh * arrK[3] +  msigm_w * msigm_w * mh* mh * mh * mh/4 + msigm_w * mh * mh * mh * sq;
	arrKExtr[0] = arrKExtr[0] + msigm_fl*msigm_fl;
	arrKExtr[1] =  arrK [1] + mh *arrK [3] + 3 * msigm_w * mh * mh * sq/2 + msigm_w * msigm_w * mh * mh * mh /2;
	arrKExtr[2] = arrKExtr[1] ;
