...
void __fastcall TForm1::Button2Click(TObject *Sender) {
	...
	const TURPointXY pntSdvig(-0.09, 0.);
	...
	TURPointXY pnt00(x1, y1);

	const double valRastigenieHor = 0.18 / (x0 - x1);
	const double valAngHor = -atan ((y1 - y0) / (x1 - x0));
	TURPolygon plgCozukh2 = plg0.LinTransform
		(valAngHor, pnt00, pntSdvig, valRastigenieHor);
	///
	 plgCozukh2.WriteSetSHPFiles(L"D:\\АМЕТИСТ_2019\\БРЛС_ПРОЧНОСТЬ\\plgCozukh2.shp",&plgCozukh2, 1 )  ;

	 plgCozukh2.WriteToASCII__(L"D:\\АМЕТИСТ_2019\\БРЛС_ПРОЧНОСТЬ\\plgCozukh2.txt");
	 return ;
