...
// создание суммарной диаграммы с углом сканирования  valTetScanRadian
...
// парам констр
  ...
  TDiagr::TDiagr(const TFar Far, const int iTypeOfDiagram, double *parrPar)
 {
  ...
  mparrCmpElCurrent[0] = exp_(-3. *delFi);
  mparrCmpElCurrent[1] = exp_(-delFi);
  mparrCmpElCurrent[2] = exp_(delFi + M_PI);
  mparrCmpElCurrent[3] = exp_(3.* delFi + M_PI);
  ...
  TDiagr::TDiagr(const TFar Far, const int iTypeOfDiagram, double *parrPar)
 {
  ...
  switch(iTypeOfDiagram)
  {
	case 1:
	   ...
	   //	{
	   //	mparrCmpElCurrent[i] = exp_(delFi0 + ((double) i) * delFi * 2. +M_PI/2. );
	  //	mparrCmpElCurrent[mFar.m_N  -1 -i] = exp_(-delFi0 - ((double) i) * delFi * 2. + M_PI +M_PI/2.);
	   //	}
...
//  mparrCmpElCurrent[0] = exp_(- delFi );//+M_PI/2.);
...
//  TComp cmpA2 = valK12* valF1 + valK22 * valF2 -  cmpSZv.m_Im ;
...
{
   // вычисление мат рицы частных ьпромизводных F по S
   double arrHs[4] = {0.};
