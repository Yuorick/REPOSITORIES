...
// данные по антенне
const double VAL_d = 2.;
const double VAL_D =32.;
const int INT_n = 16;
const int INT_N = 4;
...
// диаграмма sin(ax)/ax
double fncDiagrSimple(double valWidthDgr, double valTetMiliRad)
{
double coeff =  TET0707 / valWidthDgr/ 1000.;
return fncDiagrSinx_div_x(valTetMiliRad * coeff);
}
// производная диаграмма sin(ax)/ax
double fncDerivDiagrSimple(double valWidthDgr, double valTetMiliRad)
{
double coeff =  TET0707 / valWidthDgr/ 1000.;
return coeff* fncDerivDiagrSinx_div_x(coeff *valTetMiliRad);
}
// иторая производная диаграмма sin(ax)/ax
double fncDeriv2DiagrSimple(double valWidthDgr, double valTetMiliRad)
{
double coeff =  TET0707 / valWidthDgr/ 1000.;
return coeff * coeff* fncDeriv2DiagrSinx_div_x(coeff *valTetMiliRad);
}
...
double fncDiagrPar(double valLamb, double angScan,double tet)
{
  ...
  if (fabs(tet - angScan)< 0.0000001) return sqrt(cos(angScan));
  double temp = sqrt(cos(tet))* sin( valk * VAL_d * ((double)INT_n) * sin(tet) / 2.)
		   /(((double)INT_n)* sin( valk * VAL_d * sin(tet) / 2.)) ;
  double temp1 =  sin( valk * VAL_D * ((double)INT_N) * (sin(tet) - sin(angScan)) / 2.)
		   /(((double)INT_N)* sin( valk * VAL_D  * (sin(tet) - sin(angScan)) / 2.)) ;
  return temp * temp1 ;
}
...
double mu = sin(valTetScan);
  ...
  {
   double aa = sin( gam * ( sin( valTetScan)))/ sin( del * ( sin( valTetScan)))/((double)INT_N);
   return aa ;
  }
  }
  else
  ...
  double temp1 =  sin( gam * (sin(tet) - sin( valTetScan)) )
		   /(((double)INT_N)* sin( del* (sin(tet) - sin(valTetScan)) )) ;
  return temp * temp1 ;
}
