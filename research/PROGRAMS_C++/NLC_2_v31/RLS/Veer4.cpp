...
// оператор присваивания
 ...
 // парам констр
 __fastcall TVeer4::TVeer4(const double Tang,  double *arrScnDif, const double KSum
...
int _fastcall TVeer4::solv4(TComp *pSDifZv,TComp *pSSumZv, TComp *pZTarg, TComp *pZAnt, double *palfTrg, double *palfAnp )
{
  ...
  for (i = 0; i < 100; i++)
  {
	...
	calc_vectG_and_mtrxH (pSDifZv,pSSumZv
		...
		double valPos = arr_dFGreek[0] *arr_dFGreek[3] - arr_dFGreek[1] * arr_dFGreek[1];
	///
		...
		,  double alfTrg, double alfAnp, double* arr_FGreek, double* arr_dFGreek )
{
	...
	for (int i =0; i < 2; i++)
	{
	marrSumDgr[i].calcPartial_vectG_and_mtrxH (pSSumZv[i],alfTrg, alfAnp, valK11, valK12, valK21, valK22, arr_gradK11
	, arr_gradK12, arr_gradK21, arr_gradK22, arr_HessK11, arr_HessK12
...
void _fastcall TVeer4::calc_CoeffOtrag(TComp *pSDifZv,double alfTrg,double  alfAnp,double* valK11, double* valK12
	 ...
	 ,double* arr_gradK22, double*arr_HessK11, double* arr_HessK12, double* arr_HessK21,double* arr_HessK22)
{
   ...
   TComp cmpi(0.,1.);
   TComp cmpZero(0.,0.);
  // TComp cmpC0( marrSumDgr[0].mKSum, 0.);
 //  TComp cmpC1( marrSumDgr[1].mKSum, 0.);
...
void _fastcall TVeer4::calcFncs_Fi1_and_Fi2( TComp  cmpX1, TComp cmpX2,TComp& cmpFi1,TComp & cmpFi2)
{
...
void _fastcall TVeer4::calcVectrsGrad_Fi1_and_Fi2( TComp   cmpX1, TComp  cmpX2,TComp* cmpArrGradFi1, TComp* cmpArrGradFi2)
{
  TComp carrT0[2];
  TComp cmpNOLL(0.,0.);
  TComp  cmp_1(1., 0.);
...
void _fastcall TVeer4::calcMtrxHess_Fi1_and_Fi2( TComp  cmpX1, TComp  cmpX2
