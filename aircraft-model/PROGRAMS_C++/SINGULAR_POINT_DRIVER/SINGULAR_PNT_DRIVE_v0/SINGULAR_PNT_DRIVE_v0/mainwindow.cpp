...
///
  ...
  marrOecPosParams[0] = marrOecPosParams_Zv[0] = 0.;
  marrOecPosParams[1] = marrOecPosParams_Zv[1] = 0.;
  marrOecPosParams[2] = marrOecPosParams_Zv[2] = 0.;
...
void MainWindow:: inputData()
{
 ...
 // QString str =ui->tableWidget-> item(1,0)->text();
  ...
  // mEnvironment =TEnvironment ( Wind_V,  Wind_Alf, 0.);
   ...
   double valTargParam = (ui->doubleSpinBox_2->value()) ;
           ...
           double arrVessVS0[6] = {0.};
           double valQ0 = ui->doubleSpinBox_8->value() * M_PI / 180. ; // генеральный курс
           double valVVess = ui->doubleSpinBox_7->value();
           arrVessVS0[3] = valVVess * sin(valQ0);
           arrVessVS0[4] = valVVess * cos(valQ0);
   QInitTargData InitTargData (valR,valTargParam, valTargCourse, valV
       ,valH, mT0,arrVessVS0 );
...
//valDispAngAccel = 9.;
 ...
 arrScale[1] = arrScale[2] = arrScale[3] = arrScale[4] = 10000.;
 for (int i =1; i < 5; i++)
 {
   TYrWriteShapeFile::WriteOneReport(wchOutPutFold0// путь к папке
                                   ,arrBuff // массив с информацией - матрица nBuffRows x nBuffCols
