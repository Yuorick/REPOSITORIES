    ...
    myThread(0)
  {
   ...
   int iia = 10;
      ...
      // this->ui->lineEdit_3->setText("D:\\REPOSITORIES\\aircraft-model\PROGRAMS_C++\\AKIN\\SOLVER_POS_v6\\build-SOLVER_POS_v6-Desktop_Qt_5_12_0_MinGW_64_bit-Debug\\debug\\V000002.TXT.conv.000");
       this->ui->lineEdit_4->setText(str0);

     mqstrPrflFIle = this->ui->lineEdit_4->text();
    ...
    // str0 = str0 + "\\inputDataExchangeFile.acd";
     this->ui->lineEdit_6->setText("D:\\REPOSITORIES\\aircraft-model\\PROGRAMS_C++\\AKIN\\SOLVER_POS_v6\\build-SOLVER_POS_v6-Desktop_Qt_5_12_0_MinGW_64_bit-Debug\\debug\\beacon-3163 _yr_v2.acd");
     this->ui->lineEdit_6->setText(str0);
     mqstrDataFIle = this->ui->lineEdit_6->text();
...
void MainWindow::on_pushButton_3_clicked()
{
    ...
    QString qstr = this->ui->lineEdit_6->text();
...
/*  ui->doubleSpinBox_46->setVisible(false);
  ...
  cleanInteface();
   ...
   QString qstr = this->ui->lineEdit_6->text();
    ...
    ui->labelTaskInfo1->setText("Подготовка: предварительный отсев по геометрии...");
    ui->labelTaskInfo1->show();
    ui->labelTaskInfo2->setText("");
    ui->labelTaskInfo3->setText("");
...
// чистка замеров по геометрии
   ...
   ui->labelTaskInfo1->setText("Подготовка: предварительный отсев по невязке...");
 ...
 // построение облаков точек
     ...
     if (mqstrOutPutFold.length() >3)
     {
      ...
      _wmkdir(wchFolg0);
      ui->labelTaskInfo1->setText("Построение облаков точек по априорным данным...");
     }
...
void MainWindow::on_pushButton_8_clicked()
{
   ...
   /* if(mqstrDataFIle == nullptr)
    ...
    if (mqstrDataFIle == QString(""))
    {
       mbOpenDialog_acd = false;
       return;
    }
    mbOpenDialog_acd = true;
    this->ui->lineEdit_6->setText(mqstrDataFIle);

    if(this->ui->lineEdit_6->text().length() >3)
    {
    readHeaderDataFile(this->ui->lineEdit_6->text(),mQuantMeas0, marrAntPosParams
                                        , marrGPSPosParams, marrBeaconPos, mTObrabotki);
    fillFields();
...
void MainWindow::on_pushButton_7_clicked()
{
   ...
   // }
    ...
    this->ui->lineEdit_4->setText(mqstrPrflFIle);
    ...
    double valVmiddle = mtblEstPrfl.calcMiddleValue();

    int a = int(valVmiddle * 100.);
    ui->doubleSpinBox_49-> setValue((double(a))/100.);

    QString str("PushButton ВЫБОР <*.000>:\n " +mqstrPrflFIle +"\n ") ;
    str += QString("Средняя скорость звука = %1 м/с \n").arg((double(a))/100.);
    ui->textEdit->append(str);
    ui->textEdit->append("// !");
...
void MainWindow::on_labelTaskInfo1_objectNameChanged(const QString &objectName)
{
   ui->labelTaskInfo1->show();
}
