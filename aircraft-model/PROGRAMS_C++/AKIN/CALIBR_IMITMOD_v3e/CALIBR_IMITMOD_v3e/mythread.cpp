    ...
    int  numPart = 1;
      ...
      QLblSolver LblSolver(parrMeas, mvctMeas.size()    ,mtblEstPrfl
       ...
       // pPosSolver->calc_FGr_and_dFGr_po_dX(arrX0
                      //     ,parrF,arrMtrx_dFGr_po_dX_Inv, valSumVeviaz2__);
...
//***************************************
    ...
    mvctMeas = pPosSolver->mVectBigMeasures;
     ...
     emit USBLSucceeded(  true
        ...
        // 1
    QVector <double>vct_dAngs_po_dSgps(9, 0.);
  pSolver2D_Angs->calc_dAngs_po_dSgps(&arrXRez[5],arrMtrx_dFGrA_po_dX_Inv,&vct_dAngs_po_dSgps);//++
   qDebug() << "1. calc_dAngs_po_dSgps done" << vct_dAngs_po_dSgps.at(0);
       // 2
    QVector <double>vct_dAngs_po_dH(3,0.);
  pSolver2D_Angs->calc_dAngs_po_dHBeacon(&arrXRez[5],arrMtrx_dFGrA_po_dX_Inv,vct_dAngs_po_dH);//++
   qDebug() << "2. calc_dAngs_po_dHBeacon done";
    // 3
    QVector <double>vct_K_dqe(9,0.) ;
   pSolver2D_Angs->calc_K_qe(&arrXRez[5],arrMtrx_dFGrA_po_dX_Inv, vct_K_dqe);//+
    qDebug() << "3. calc_K_qe done";
    // 4
    QVector <double>vct_dAngs_po_dAngSins(9,0.) ;// ошибка
    ...
    // 5
    QVector <double>vct_dAngs_po_dProfile(3,0.);
   pSolver2D_Angs->calc_dAngs_po_dProfile(&arrXRez[5],arrMtrx_dFGrA_po_dX_Inv
          ,arr_dX_po_dProfile,vct_dAngs_po_dProfile);//- ??
    qDebug() << "5. calc_dAngs_po_dProfile done";
    // 6
    QVector <double>vct_AngKgps(9);// ошибка
