...
// Конструктор копирования
 ...
 TURPointXY &TURPointXY::operator=(const TURPointXY  &R2)
{
	 X= R2.X ;
	  Y= R2.Y;
	 return *this ;
...
//--------------------------------------------------------
//---------------------------------
int TURPointXY::PutPointsToCsvFile_(wchar_t*FileName,TURPointXY *urpntP,
                                   const int lenArray)
{


 double *parrBuff = new double [2 * lenArray];
 for(int i =0; i <lenArray; ++i)
 {
   parrBuff[2 * i ] = urpntP[i].X;
   parrBuff[2 * i +1] = urpntP[i].Y;
 }

 TYrWrite::WriteMassiveInFIleSCV(FileName,parrBuff, lenArray, 2
                              ,NULL,NULL, 0);
...
// Чистка массива точек достижимости от близких точек
// Ecли разница координат по X и по Y меньше  valEps, то точка удаляется
...
//---------------------------------------------------------
// удаление из массива точек повторяющихся точек
// задан исходный массив точек parrPnts0 длиной  lenarr0
// этот массив может содержать повторяющиеся точки
// Точки считаются одинаковыми, если расстояние между ними менее  VAlTolerance
// ФУнкция удаляет лишние точки из массива parrPnts0
// В результате работы функции в первых  *plenarr элементов массива parrPnts0
// сохранияются неповторяющиеся точки
// остальные элементы массива(высвобождающиеся) запролняются нулями
//
...
// сортировка массива точек в порядке возрастания координаты X
...
//-------------------------------------------
void TURPointXY::swap(TURPointXY &pnt0, TURPointXY &pnt1)
{
  TURPointXY pnt = pnt0;
  pnt0 = pnt1;
  pnt1 = pnt;
