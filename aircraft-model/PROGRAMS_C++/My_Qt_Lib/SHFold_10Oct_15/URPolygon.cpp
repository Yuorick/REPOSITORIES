...
TURPolygon::TURPolygon(  const int iNumParts,const int iNumPoints,const int *iarrParts
                    ,const double *arr)
{
...
TURPolygon::TURPolygon (QPolygonF &qplg)
{
    NumParts = 1 ;
    //3
    NumPoints = qplg.size(); ;
...
bool TURPolygon::ReadPolygonFromASCII(wchar_t*FileName)
{
 ...
 // функции   члены
 // возвращает true если точка v находится строго внутри полигона и false в противнм случае
...
//-------------------------------------------------
// нахождение центра масс полигона
TURPointXY TURPolygon::calcCentreMass(const double cs)
{
    TYrRastr rastr( *this,  cs,  1.) ;
    int isum = 0;
    double xsum = 0.;
    double ysum = 0.;
    for (int i = 0; i <  rastr.nrows; i++)
    for (int j = 0; j < rastr.ncols ; j++)
    {
       if (rastr.pflt_rastr[ i * rastr.ncols + j]  <0.) continue;
       xsum +=  rastr.get_X(j);
       ysum +=  rastr.get_Y(i);
       isum++;
    }
    return  TURPointXY(xsum/ ((double)isum), ysum/ ((double)isum));
}


//-------------------------------------------------
// нахождение статического момента площади полигона относительно оси OY
double TURPolygon::calcSy(const double cs)
{
    TYrRastr rastr( *this,  cs,  1.) ;
    int isum = 0;
...
//-------------------------------------------------
// нахождение статического момента площади полигона относительно оси OX
double TURPolygon::calcSx(const double cs)
{
    TYrRastr rastr( *this,  cs,  1.) ;
    int isum = 0;
...
//-------------------------------------------------
// нахождение  момента инерции площади полигона относительно оси
// параллельной оси OY и проходящей через точку [X0;0]
double TURPolygon::calcJy(const double cs, const double VAlX0)
{
    TYrRastr rastr( *this,  cs,  1.) ;
    int isum = 0;
    ...
    {
       if (rastr.pflt_rastr[ i * rastr.ncols + j]  <0.) continue;
       double temp = rastr.get_X(j) -VAlX0;
       xsum +=  temp * temp;
       isum++;
    }
    return  xsum *  cs * cs;
...
//-------------------------------------------------
// нахождение  момента инерции площади полигона относительно оси
// параллельной оси OY и проходящей через точку [0;Y0]
double TURPolygon::calcJx(const double cs, const double VAlY0)
{
    TYrRastr rastr( *this,  cs,  1.) ;
    int isum = 0;
    ...
    {
       if (rastr.pflt_rastr[ i * rastr.ncols + j]  <0.) continue;
       double temp = rastr.get_Y(i) -VAlY0;
       xsum +=  temp * temp;
       isum++;
    }
    return  xsum *  cs * cs;
...
// нахождение иненрционной матрицы
void TURPolygon::calcInertiaMtrx(const TURPointXY pntCentre, const double cs, double *mtrxInertia)
{

    TYrRastr rastr( *this,  cs,  1.) ;
    double arrCentre[2] = {0.}, arrSum[4] = {0.}, arrTemp0[2] = {0.}
          , arrTemp1[2] = {0.}, arrTemp2[4] = {0.}, arrTemp3[4] = {0.};
    arrCentre [0] = pntCentre.X;
    arrCentre [1] = pntCentre.Y;
    int isum = 0;
    double xsum = 0.;
    double ysum = 0.;
    for (int i = 0; i <  rastr.nrows; i++)
    ...
    {
       if (rastr.pflt_rastr[ i * rastr.ncols + j]  <0.) continue;
       arrTemp0[0] =  rastr.get_X(j);
       arrTemp0[1] =  rastr.get_Y(i);
       MtrxMinusMatrx(arrTemp0, arrCentre,1, 2, arrTemp1);
       memcpy(arrTemp0, arrTemp1, 2 * sizeof(double));
       MtrxMultMatrxTransp(arrTemp0,2, 1, arrTemp1,2, arrTemp2) ;
       MtrxSumMatrx(arrSum, arrTemp2,2, 2, arrTemp3) ;
       memcpy(arrSum, arrTemp3, 4 * sizeof(double));
       isum++;
    }
     if (isum == 0)
     {
     ...
     MatrxMultScalar(arrSum, 2, 2, cs *cs ,mtrxInertia);

}

//-------------------------------------------------
// создагште овала Кассини с осями параллельныими осям координат
//INPUT:
//vala - квадр корень из произведения расстояния
//valc - полоовина фокусного расстояния
// 2 * numPoints +1 - к-во точек
TURPolygon  TURPolygon::createCassiniOval(const double vala, const double valc
     , const int numPoints )
{
  TURPointXY *pnts = new TURPointXY[numPoints *2 -1];
  double valx0 = sqrt(vala * vala + valc * valc);
  pnts[0].X = -valx0;
  pnts[0].Y = 0.;
  pnts[numPoints-1].X = valx0;
  pnts[numPoints-1].Y = 0.;
  pnts[numPoints *2 -2] = pnts[0];
  for (int i= 1; i < numPoints-1 ; i++)
  {
    pnts[i].X = -valx0 + 2. * valx0/((double)( numPoints-1.)) * ((double)i) ;

    pnts[i].Y = sqrt(sqrt(vala * vala * vala * vala + 4. *valc * valc *pnts[i].X * pnts[i].X )
                     - pnts[i].X * pnts[i].X -valc * valc);
    pnts[2 *numPoints -2 - i].X = pnts[i].X;
    pnts[2 *numPoints -2 - i].Y = -pnts[i].Y;
  }
...
// Создание  окружности с центром в точке  pointCentre  и
// радиусом  valR с количеством точек  iNUM
TURPolygon TURPolygon::fncCreateEllipse_(const TURPointXY pointCentre
     ,const double a,const double b, const int iNUM)
{
 ...
 for (int i =0; i < iNUM; i++)
 {
   parrPoints[i].X = a * cos(valFiCur) + pointCentre.X ;
   parrPoints[i].Y = b * sin(valFiCur) + pointCentre.Y ;
   valFiCur += valDelFi ;
 }
 parrPoints[iNUM ] = parrPoints[0] ;
 TURPolygon plgRez = TURPolygon(iNUM + 1, parrPoints);
